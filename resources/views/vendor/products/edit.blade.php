...
<form action="{{ route('vendor.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6" x-data="imageUpload({{ json_encode($product->images ?? []) }})">
    @csrf
    @method('PUT')
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div x-show="existingImages.length > 0" class="mb-4">
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <template x-for="(image, index) in existingImages" :key="'existing-' + index">
                                <div class="relative group">
                                    <img :src="image" class="w-full h-32 object-cover rounded-xl border border-gray-200">
                                    <button 
                                        type="button"
                        ...
                        <template x-for="(image, index) in existingImages" :key="'input-' + index">
                    ...
                    <!-- Upload Area -->
                    <div>
                        <p class="text-sm font-medium text-gray-700 mb-3">Add New Images</p>
                        <div 
                    ...
                    <div x-show="previews.length > 0" class="grid grid-cols-3 gap-4">
                        <template x-for="(preview, index) in previews" :key="index">
                            <div class="relative group">
                                ...
                                <span class="absolute bottom-2 left-2 bg-green-500 text-white text-xs px-2 py-1 rounded">
                                    New
                                </span>
                            </div>
                        </template>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        >
                        <p class="text-xs text-gray-500 mt-2">Unique product identifier</p>
                        @error('sku')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
...
<script>
function imageUpload(existingImages = []) {
    return {
        dragOver: false,
        files: [],
        previews: [],
